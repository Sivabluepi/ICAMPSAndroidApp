...
object LockType {
    ...
    fun getCurrent(contentResolver: ContentResolver?): Int {
        ...
        return when (mode) {
            ...
            DevicePolicyManager.PASSWORD_QUALITY_ALPHANUMERIC.toLong() -> {
                PASSWORD_ALPHANUMERIC
            }
            DevicePolicyManager.PASSWORD_QUALITY_ALPHABETIC.toLong() -> {
                PASSWORD_ALPHABETIC
            }
            DevicePolicyManager.PASSWORD_QUALITY_NUMERIC.toLong() -> {
                PIN
            }
            else -> SOMETHING_ELSE
        }
